      ...
      <div class="modal-body-custom">
        <form inline @submit.stop.prevent>
          ...
          <footer class="modal-card-foot">
            ...
            />
            <b-button
              :label="$t('my-products.popup.save')"
              type="is-primary"
              @click="submit"
            />
          </footer>
  ...
  methods: {
    ...
    submit() {
      ...
      if (this.data.name === null || this.data.name === '') {
        this.error_server = this.$t('my-products.popup.errors.name')
        return
      }
      if (this.data.barcode === null || this.data.barcode === '') {
        this.error_server = this.$t('my-products.popup.errors.barcode')
        return
      }
      if (this.data.price === null || this.data.price === '') {
        this.error_server = this.$t('my-products.popup.errors.price')
        return
      }
      if (this.data.quantity === null || this.data.quantity === '') {
        this.error_server = this.$t('my-products.popup.errors.quantity')
        return
      }
      this.isLoading = true

      this.$axios
        .put(`/products/${this.$route.params.id}`, this.data, {
          headers: {
            Authorization: `Bearer ${this.$auth.strategy.token.get()}`,
            'Content-Type': 'application/json',
          },
    ...
    success() {
      this.$buefy.notification.open({
        message: this.$t('my-products.notifications.product_updated'),
        type: 'is-success',
      })
    },
    closeEditpage() {
